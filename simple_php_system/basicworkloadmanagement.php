...
<div class="popup" id="popup">
    ...
    <div class="container mt-5">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="subjectCodelist" name="subjectCodelist">
                            ...
                            <div class="mb-1">
                                ...
                                echo '<div class="mb-1">';
                                echo '<label for="selectedSubjects" class="form-label">Programme:</label>';
                                while ($row = mysqli_fetch_assoc($result)) {
        ...
        <?php 
            ...
            if (isset($_POST['submit'])) {
                ...
                $accumulatedTotalWorkload = 0;

                $result = mysqli_query($con, "SELECT * FROM workload WHERE lecturerID = '$selectedLecturerValue'");

                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        // Get the total workload for the current record
                        $totalWorkload = (int) $row['totalWorkload'];

                        // Add the current total workload to the accumulated total workload
                        $accumulatedTotalWorkload += $totalWorkload;

                        // You can also display the details of the record if needed
                        //echo "Lecturer: " . $row['lecturerID'] . ", Total Workload: " . $totalWorkload . "<br>";
                    }

                    echo "Accumulated Total Workload for lecturer with ID;   $selectedLecturerValue: $accumulatedTotalWorkload";

                    if ($accumulatedTotalWorkload > 20) {

                        ?>

                        <!--If accumulated workload exceeds 20, show a confirmation form-->
                        <div class="modal" id="confirmationModal" tabindex="-1" role="dialog">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        Accumulated workload is now ' . $accumulatedTotalWorkload . ', do you still want to add?
                                        </div>
                                        <div class="modal-footer">
                                        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                                        <input type="hidden" name="selectedLecturerValue" value="' . $selectedLecturerValue . '">
                                        <input type="hidden" name="accumulatedTotalWorkload" value="' . $accumulatedTotalWorkload . '">
                                        <button type="submit" name="confirm" class="btn btn-primary">Yes, I want to add</button>
                                        <button type="button" name="cancel" class="btn btn-secondary" data-dismiss="modal">No, cancel</button>
                                        </form>
                                        </div>
                                    </div>
                                </div>
                        ...
                        if(isset($_POST['confirm'])) {
                            ...
                            // Proceed with the code to add the record
                            ...
                            //$contactHours = $selectedContactHours;
                            ...
                            $otherRoles = json_encode($numberedRoles);
                                ...
                                $query = "INSERT INTO workload (lecturerID, subjectCode, programme, level, semester, totalStudent, totalContactHours, mentor, supervising, totalWorkload, teachingPeriod, otherRoles)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                                ...
                                $stmt = mysqli_prepare($con, $query);
                                    ...
                                    if ($stmt === false) {
                                        ...
                                        mysqli_stmt_bind_param($stmt, "sssssiiiiiss", $lecturerID, $subjectCode, $programme, $level, $semester, $totalStudent, $totalContactHours, $mentor, $supervising, $totalWorkload, $teachingPeriod, $otherRoles);
                                    
                                        if (mysqli_stmt_execute($stmt)) {
                                            //echo "Workload added successfully.";
                                            echo '<script>alert("Workload for' . $lecturerID . ' added successfully.");</script>';
                                        } else {
                                ...
                                header("Location: basicworkloadmanagement.php"); // Replace with the actual URL
                    ...
                    echo '<option value="">No Records Found for the specified lecturer.';
                }*/
    ...
    icon.addEventListener('click', function() {
        ...
        const subjectCode = row.cells[2].textContent;
        const programme = row.cells[3].textContent;
        const level = row.cells[4].textContent;
        const semester = row.cells[5].textContent;
        const totalStudent = row.cells[6].textContent;
        const totalContactHours = row.cells[7].textContent;
        const mentor = row.cells[8].textContent;
        const supervising = row.cells[9].textContent;
        const totalWorkload = row.cells[10].textContent;
        const teachingPeriod = row.cells[11].textContent;
        const otherRoles = row.cells[12].textContent;
        ...
        // Assuming you have retrieved the roles from the database as an array
        const rolesArray = otherRoles.split(', ');

        // Select the roles container
        const rolesContainer = document.querySelector('.roles-container');

        // Iterate over the roles array to create and append role input fields
        ...
        // Update the roleCount to the number of roles
        roleCount = rolesArray.length;

        // Add the 'Add Another Role' button to allow users to add more roles
        ...
        if (confirmed) {
            // Extract data from the row cells
            const lecturerID = row.cells[0].textContent;

            // Create a form element for deletion
            const deleteForm = document.createElement('form');
            deleteForm.method = 'post';
            deleteForm.action = 'basicworkloadmanagement.php';

            // Create hidden input fields for action and lecturerID
            const actionInput = document.createElement('input');
            actionInput.type = 'hidden';
            actionInput.name = 'action';
            actionInput.value = 'DeleteWorkload';

            const lecturerIDInput = document.createElement('input');
            lecturerIDInput.type = 'hidden';
            lecturerIDInput.name = 'lecturerID';
            lecturerIDInput.value = lecturerID;

            // Append inputs to the form
            deleteForm.appendChild(actionInput);
            deleteForm.appendChild(lecturerIDInput);
    ...
    $(document).ready(function () {
        ...
        <?php if ($accumulatedTotalWorkload > 20) { ?>
